...
public class LudoGameController : PunBehaviour , IMiniGame
{
    ...
    public void HighlightPawnsToMove(int player, int steps)
    {
        ...
        if (steps == 6)
        {
            ...
            if (SixStepsCount == 3)
            {
                ...
                if (GameGui != null)
                {
                    ...
                    Invoke("sendFinishTurnWithDelay", 0.2f);
                }
        ...
        else
        {
            ...
            {
                if (possiblePawns > 0 && GameManager.Instance.currentPlayer.isBot && !GameManager.Instance.isLocalPLay)
                {
    ...
    public void RollDiceWithDelay()
    {
        if (GameManager.Instance.isLocalPLay)
            return;
        int e = Random.Range(1,7);
        GameManager.Instance.currentPlayer.dice.GetComponent<GameDiceController>().RollDiceBot(e);
    }
